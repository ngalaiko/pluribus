...
and have recursion depth four. `background_model_calls_per_hour` defaults to 64
and persists across jobs and restarts. Calls count at admission, so failed and
cancelled attempts consume budget, and neither restart nor job creation resets
the rolling allowance. Exhaustion schedules the earliest eligible wake instead of
polling the model.
