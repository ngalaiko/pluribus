...
| E2E-07 | `cargo build --locked -p pluribus-plugin-http --bin pluribus-http-listener`; `cargo test --locked -p pluribus-runtime-wasm --test github_delivery -- --include-ignored` | Signed webhook passes through the public listener process and packaged Wasm; secrets, enrollment replay, and token export checks pass. | Local listener and fake credentials | Not run |
