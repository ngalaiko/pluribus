...
{
  "model": "gpt-5.6-luna",
  "identity": "Answer tersely.",
  "connectors": ["telegram-1/receive"],
  "trusted_users": ["123"],
  "tools": [],
...
`connectors` lists the component selectors whose observations count as user
input. A reply goes to `<provider>.reply` for the provider the observation
carries, so cognition needs no connector-specific configuration. Set the
background ceiling in that instance's configuration. No credential changes are
needed. The package contains `cognition` and its required `js` component, one crate each under `plugins/rlm`; no separate JS installation is needed.

JS operations:

- `history.read({after, limit})`: trusted root history or a delegated child range;
...
Children inherit the job revision and background allowance. Delegated history
...
input makes no idle model calls. Background accounting and wakes survive restart.
