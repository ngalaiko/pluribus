...
#[test]
fn continuation_uses_owned_revision_and_persisted_rolling_allowance() {
    let mut e = Engine::default();
    let mut c = config();
    ...
    assert_eq!(paused[0].payload["dueAtMs"], 3_660_000);
    assert!(!paused.iter().any(|d| d.kind == "model.requested"));
}
#[test]
...
#[test]
fn untrusted_reference_cannot_amend_a_trusted_job() {
    let mut e = Engine::default();
    let c = config();
    ...
    observe(
        ...
        &c,
        "attack",
        json!({"jobId":"one","externalSenderId":"intruder"}),
    );
