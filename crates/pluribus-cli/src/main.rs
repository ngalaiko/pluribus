...
async fn example_configuration(data: &Paths) -> Result<(), Box<dyn Error>> {
    ...
    config.trusted_senders = vec!["operator".into()];
...
async fn credential_target(
    ...
    Ok(CredentialTarget {
        package: resolve_plugin(data, &instance.package).await?,
        instance_id: id.into(),
                ...
                components: declaration
                    ...
                    .map(|name| {
                        PrincipalRef::new(
                            ...
                            format!("{}/{name}", target.instance_id),
                        )
                    })
                    .collect(),
                injection_origins,
...
/// Fills the tool catalog of every configured RLM instance that omits one.
async fn refresh_cognition_tools(data: &Paths, config: &mut Config) -> Result<(), Box<dyn Error>> {
    ...
    for (id, instance) in &config.plugin_instances {
        ...
        for (name, component) in package.components() {
            for capability in &component.manifest().provides {
                ...
                let provider = PrincipalRef::new(PrincipalKind::Component, format!("{id}/{name}"));
    ...
    for id in rlm_instances {
        ...
        if !instance
            .config
            .as_object()
            .is_some_and(|config| config.contains_key("tools"))
        {
...
async fn run_agent(data: &Paths, offline: bool) -> Result<(), Box<dyn Error>> {
    ...
    for id in config.plugin_instances.keys() {
        ...
        for name in package.components().keys() {
            enrollment_policies.push((
                PrincipalRef::new(PrincipalKind::Component, format!("{id}/{name}")),
                target
                    .enrollment_origins
                    .iter()
    ...
    let mut agent = Agent::new(
        ...
        pluribus_cognition::OriginAuthority {
            ...
            connectors,
            trusted_senders: config.trusted_senders.clone(),
        },
        runtime,
        Arc::clone(&database) as Arc<dyn EventStore>,
        stream_id,
    ...
    for (id, instance) in &config.plugin_instances {
        ...
        for (name, component) in package.components() {
            ...
            let component_id = format!("{id}/{name}");
...
mod tests {
    ...
    async fn auth_resolves_an_external_instance_from_configuration() {
        ...
        value["plugin_instances"] = json!({
            "mail-work": {
                "package": "file:///tmp/agent/plugins/mail",
        ...
        assert_eq!(target.package, PathBuf::from("/tmp/agent/plugins/mail"));
