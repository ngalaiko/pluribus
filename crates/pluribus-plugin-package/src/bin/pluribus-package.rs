...
fn main() -> ExitCode {
    ...
    if arguments.len() < 3 {
        eprintln!(
            "usage: pluribus-package <template-dir> <output-dir> <core-module.wasm | name=core-module.wasm>..."
        );
        return ExitCode::from(2);
    }
    let mut modules = BTreeMap::new();
    for binding in &arguments[2..] {
        let (name, path) = binding.split_once('=').unwrap_or(("", binding));
        if path.is_empty() || modules.insert(name.into(), PathBuf::from(path)).is_some() {
